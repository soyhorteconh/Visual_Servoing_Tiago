#include <cmath>
#include <stdexcept>
#include <inria_maths/LinearRegression.hpp>
    ...
    const Eigen::DiagonalMatrix<double, Eigen::Dynamic>& regWeight)
{
    if (
        ...
        throw std::logic_error(
            "inria::LinearRegression::setRegularization: "
...
const Eigen::MatrixXd& LinearRegression::fit()
{
    ...
    if (count() == 0 || dimIn() == 0) {
        throw std::logic_error(
            "inria::LinearRegression::fit: Empty data");
    }
    ...
    const Eigen::VectorXd& input) const
{
    if (input.size() != _params.cols()) {
        throw std::logic_error(
            "inria::LinearRegression::predict: "
            "Model not fitted.");
    }
...
Eigen::MatrixXd LinearRegression::residuals() const
{
    if (count() == 0 || dimIn() == 0) {
        throw std::logic_error(
            "inria::LinearRegression::residuals: "
            "Empty data.");
    }
