#ifndef INRIA_MATHS_TIMESERIES_HPP
#define INRIA_MATHS_TIMESERIES_HPP
...
class TimeSeries
{
    public:
        ...
        inline const Point& operator[](size_t index) const
        {
            if (index >= size()) {
                throw std::logic_error(
                    "inria::TimeSeries::at: Invalid index: " 
                    + std::to_string(index));
            }
        ...
        inline void append(double time, const T& value)
        {
            if (std::isnan(time)) {
                throw std::logic_error(
                    "inria::TimeSeries::append: Adding NaN time.");
            }
            if (_data.size() > 0 && time <= lastTime()) {
                throw std::logic_error(
                    "inria::TimeSeries::append: Adding past time.");
            }
    ...
    private:
            ...
            size_t& indexLow, size_t& indexUp) const
        {
            if (size() < 2) {
                throw std::logic_error(
                    "inria::TimeSeries:bisectionSearch: "
                    "Not enough points.");
            }
