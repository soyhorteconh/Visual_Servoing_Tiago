...
#include <inria_maths/FilterEncoderDifferentiation.hpp>
#include <inria_maths/Sign.h>
...
void testEncoderDifferentiation()
{
    ...
    inria::FilterEncoderDifferentiation filter;
...
void testFilteringWithControl()
{
    ...
    inria::FilterExponential<double> filterPos(5.0);
    inria::FilterExponential<double> filterVel(5.0);
    inria::FilterExponential<double> filterControl(50.0);
    for (double t=0.0;t<12.0;t+=dt) {
        ...
        double acc2 = 1.0/mass2*(
            ...
            - frictionStatic*inria::Sign(vel2)
            - frictionVel*vel2);
        double acc1 = 1.0/mass1*(
            - stiffness2*(pos1-pos2) 
            - mass1*9.81*std::sin(pos1) 
            - frictionStatic*inria::Sign(vel1) 
            - frictionVel*vel1 
            + filterControl.value());
        vel1 += dt*acc1;
        vel2 += dt*acc2;
