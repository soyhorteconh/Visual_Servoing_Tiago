#ifndef INRIA_PLOT_PLOT_HPP
#define INRIA_PLOT_PLOT_HPP
...
class Plot
{
    public:
            ...
            if (yAxis == "all") {
                ...
                if (
                    ...
                    throw std::logic_error(
                        "inria::Plot::plot: Style errors needs a third column.");
                }
                if (style == Vectors) {
                    throw std::logic_error(
                        "inria::Plot::plot: Style vector needs 4 columns.");
                }
                Plot2D request;
                request.xAxis = xAxis;
                request.yAxis = yAxis;
            ...
            const std::string& palette = "")
        {
            ...
            if (zAxis == "all") {
                ...
                if (
                    ...
                    throw std::logic_error(
                        "inria::Plot::plot: Style errors not implemented for 3d.");
                }
                if (
                    style == Vectors && 
                    palette == ""
                    ...
                    throw std::logic_error(
                        "inria::Plot::plot: Style vectors needs 4 columns.");
                }
                Plot3D request;
                request.xAxis = xAxis;
                request.yAxis = yAxis;
        ...
        /**
         * Set Gnuplot terminal name
         */
        Plot& terminal(const std::string& name)
        {
            _terminal = name;
        ...
        /**
         * Set optional cutom Gnuplot styling commands
         */
        Plot& custom(const std::string& cmds)
        {
            _customCmds = cmds;
        ...
        Plot& multiplot(unsigned int row, unsigned int col)
        {
            ...
            if (_plots3D.size() != 0) {
                throw std::logic_error(
                    "inria::Plot::multiplot: 3d plots invalid with multiplot.");
            }
        ...
        Plot& nextPlot()
        {
            ...
            if (_plots3D.size() != 0) {
                throw std::logic_error(
                    "inria::Plot::nextPlot: 3d plots invalid with multiplot.");
            }
            if (_plots2D.size() > 0) {
            ...
            const std::string& exportFile = "AUTO")
        {
            ...
            if (
                ...
                throw std::logic_error(
                    "inria::Plot::show: 2d and 3d requests are mixed.");
            }
            ...
            generatePlotting(outStream, false);
            std::string commands = outStream.str();
        ...
        void stream()
        {
            ...
            if (
                ...
                throw std::logic_error(
                    "inria::Plot::stream: 2d and 3d requests are mixed.");
            }
            if (_plots2D.size() == 0 && _plots3D.size() == 0) {
                return;
            }
            ...
            std::ostringstream outStream;
            generatePlotting(outStream, true);
            std::string commands = outStream.str();
            ...
            if (_pipeFd <= 0) {
                throw std::logic_error(
                    "inria:Plot::stream: Closed pipe.");
            }
            ssize_t sizeWriten = write(_pipeFd, commands.c_str(), commands.length());
            if (sizeWriten != (ssize_t)commands.length()) {
                throw std::logic_error(
                    "inria:Plot::stream: Write error.");
            }
        ...
        void closeWindow()
        {
            ...
            if (sizeWriten != (ssize_t)commands.length()) {
                throw std::logic_error(
                    "inria:Plot::closeWindow: Write error.");
            }
            if (_pipeFd != -1) {
                close(_pipeFd);
                _pipeFd = -1;
    ...
    private:
        ...
        /**
         * Plot optional additional Gnuplot commands
         */
        std::string _customCmds;
        ...
        void createGnuplotInstance()
        {
            ...
            if (pipe(pipefd) == -1) {
                throw std::runtime_error(
                    "inria::Plot::createGnuplotInstance: Failed to create pipe.");
            }
            ...
            if (pid > 0) {
                ...
                if (dup2(pipefd[0], STDIN_FILENO) == -1) {
                    throw std::runtime_error(
                        "inria::Plot::createGnuplotInstance: Failed to dup2.");
                }
                ...
                throw std::runtime_error(
                    "inria::Plot::createGnuplotInstance: Failed to fork.");
            }
        }
        ...
        /**
         * Shorthand to write given value into stream as binary float32
         */
        template <typename T>
        static void writeBinaryStream(std::ostringstream& oss, const T& val)
        {
            float tmpVal = (float)val;
            oss.write((char*)&tmpVal, sizeof(float));
        }

        /**
         * Generate and assign Gnuplot commands and data
         *
         * @param outStream Output stream into write commands and binary data.
         * @param isStream If true, the window is not closed
         * at the end of command. Use for interactive plotting.
         */
        void generatePlotting(std::ostringstream& outStream, bool isStream)
        {
            if (isStream) {
                outStream << "set terminal " << _terminal << " noraise size ";
            } else {
                outStream << "set terminal " << _terminal << " size "; 
            }
            outStream << _winWidth << "," << _winHeight << ";\n"; 
            generateStyle(outStream);
            
            //Custom additional commands
            outStream << _customCmds;
            ...
            if (isMultiplot) {
                outStream << "set multiplot layout "
                    << _multiplotLayoutRow << ", "
                    << _multiplotLayoutCol << ";\n";
            }
            ...
            while (true) {
                ...
                if (_rangeMinX < _rangeMaxX) {
                    commands << "set xrange[" << std::setprecision(15) << _rangeMinX;
                    commands << ":" << std::setprecision(15) << _rangeMaxX << "];\n";
                }
                if (_rangeMinY < _rangeMaxY) {
                    commands << "set yrange[" << std::setprecision(15) << _rangeMinY;
                    commands << ":" << std::setprecision(15) << _rangeMaxY << "];\n";
                }
                if (_rangeMinZ < _rangeMaxZ) {
                    commands << "set zrange[" << std::setprecision(15) << _rangeMinZ;
                    commands << ":" << std::setprecision(15) << _rangeMaxZ << "];\n";
                }
                if (_title.length() > 0) {
                ...
                for (
                    ...
                    for (size_t j=0;j<_database.size();j++) {
                        ...
                        if (_plots2D[i].xAxis == "index") {
                            writeBinaryStream(data, j);
                            writeBinaryStream(data, _database[j].at(_plots2D[i].yAxis));
                        } else {
                            writeBinaryStream(data, _database[j].at(_plots2D[i].xAxis));
                            writeBinaryStream(data, _database[j].at(_plots2D[i].yAxis));
                        }
                        if (isPalette) {
                            if (_plots2D[i].palette == "index") {
                                ...
                                writeBinaryStream(data, _database[j].at(_plots2D[i].palette));
                            }
                        }
                        sizeRows++;
                    }
                    ...
                    if (isPalette) {
                        if (
                            ...
                            commands << "'-' binary format='\%float32' record=" 
                                << sizeRows << " using 1:2:3 with ";
                        } else {
                            commands << "'-' binary format='\%float32' record=" 
                                << sizeRows << " using 1:2:3 palette with ";
                        }
                    } else {
                        commands << "'-' binary format='\%float32' record=" 
                            << sizeRows << " using 1:2 with ";
                    }
                ...
                for (size_t i=0;i<_plots3D.size();i++) {
                    ...
                    for (size_t j=0;j<_database.size();j++) {
                        ...
                        if (_plots3D[i].xAxis == "index") {
                            ...
                            writeBinaryStream(data, _database[j].at(_plots3D[i].xAxis));
                        }
                        if (_plots3D[i].yAxis == "index") {
                            writeBinaryStream(data, j);
                        } else {
                            writeBinaryStream(data, _database[j].at(_plots3D[i].yAxis));
                        }
                        if (_plots3D[i].zAxis == "ZERO") {
                            writeBinaryStream(data, 0.0);
                        } else {
                            writeBinaryStream(data, _database[j].at(_plots3D[i].zAxis));
                        }
                        if (isPalette) {
                            if (_plots3D[i].palette == "index") {
                                writeBinaryStream(data, j);
                            } else {
                                writeBinaryStream(data, _database[j].at(_plots3D[i].palette));
                            }
                        }
                        sizeRows++;
                    }
                    ...
                    if (_plots3D[i].style == Vectors) {
                        commands << "'-' binary format='\%float32' record=" 
                            << sizeRows << " using 1:2:($4*cos($3)):($4*sin($3)) with ";
                    } else if (isPalette) {
                        commands << "'-' binary format='\%float32' record=" 
                            << sizeRows << " using 1:2:3:4 palette with ";
                    } else {
                        commands << "'-' binary format='\%float32' record=" 
                            << sizeRows << " using 1:2:3 with ";
                    }
            ...
            if (isMultiplot) {
                outStream << "unset multiplot;\n";
            }
            if (!isStream) {
                outStream << "pause mouse close;\nquit;\nquit;\n";
            } 
        }

        /**
         * @param outStream Assign a color-blind 
         * palette for Gnuplot style
         */
        void generateStyle(std::ostringstream& outStream) const
        {
            outStream << 
                "set style line 1 lt 1 lc rgb '#000000' lw 1;\n"
                "set style line 2 lt 1 lc rgb '#E69F00' lw 1;\n"
                "set style line 3 lt 1 lc rgb '#56B4E9' lw 1;\n"
                "set style line 4 lt 1 lc rgb '#009E73' lw 1;\n"
                "set style line 5 lt 1 lc rgb '#CC79A7' lw 1;\n"
                "set style line 6 lt 1 lc rgb '#0072B2' lw 1;\n"
                "set style line 7 lt 1 lc rgb '#D55E00' lw 1;\n"
                "set style line 8 lt 1 lc rgb '#F0E442' lw 1;\n"
