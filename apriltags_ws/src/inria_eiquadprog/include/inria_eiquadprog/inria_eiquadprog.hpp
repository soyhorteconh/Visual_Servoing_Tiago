...
//Source: http://www.labri.fr/perso/guenneba/code/QuadProg/
//TODO XXX try to resync this file and the original QuadProg++ with updated code
...
inline void assign_duals(
    VectorXd* dualEq, 
    VectorXd* dualIneqActive, 
    VectorXi* dualIneqIndex, 
    int p, int iq, const VectorXi& A, const VectorXd u)
{
    //Variables documentation:
    //p: Number of equality constraints
    //m: Number of inequality constraints
    //iq: Number of current active constraints 
    //(equalities and inequalities)
    //A: Mapping from index in u to associated constraints as 
    //defined in problem matrices.
    //u: Lagrangian duals. First the equality constraints 
    //in same order as defined. Then inequality constraints ordered by
    //mapping in A. Only the first iq entries are valid.

    //Assign all equality duals
    if (dualEq != nullptr) {
        *dualEq = u.segment(0, p);
    }
    //Assign active inequality duals
    if (dualIneqActive != nullptr) {
        *dualIneqActive = u.segment(p, iq-p);
    }
    //Assign index mapping from active 
    //inequalities to defined inequalities in matrix
    if (dualIneqIndex != nullptr) {
        *dualIneqIndex = A.segment(p, iq-p);
    }
}
...
inline double solve_quadprog(MatrixXd & G,  VectorXd & g0,  
                      ...
                      VectorXd& x,
                      VectorXd* dualEq = nullptr,
                      VectorXd* dualIneqActive = nullptr,
                      VectorXi* dualIneqIndex = nullptr)
{
  using std::abs;
  int i, j, k, l; /* indices */
  (void)j;
  ...
  iq = 0;
    ...
    if (!add_constraint(R, J, d, iq, R_norm))
    {
      ...
      assign_duals(
          ...
          p, iq, A, u);
      throw std::runtime_error("eiquadprog::solve_quadprog: Equality constraints linearly dependent.");
      return f_value;
    }
  }
