...
#include <stdexcept>
#include <algorithm>
#include <inria_utils/Chrono.hpp>
...
void Chrono::stop(const std::string& name)
{
    if (_durations.count(name) == 0) {
        throw std::logic_error(
            "inria::Chrono::stop: Invalid stop: " 
            + name);
    }
...
void Chrono::print(std::ostream& os) const
{
    ...
    children["inria::Chrono::ROOT"] = std::vector<std::string>();
    ...
    while (true) {
        ...
        if (isStart) {
            ...
            for (const auto& n : openedNames) {
                if (minName == n) {
                    throw std::logic_error(
                        "inria::Chrono:print: Started twice: " 
                        + minName);
                }
            }
        ...
        //If the next time point is a stop()
            ...
            if (openedNames.front() != minName) {
                throw std::logic_error(
                    "inria::Chrono:print: Not well parenthesized: " 
                    + minName);
            }
    ...
    printHierarchy(os, "inria::Chrono::ROOT", 0, children, "");
}
...
double Chrono::mean(const std::string& name) const
{
    if (_durations.count(name) == 0) {
        throw std::logic_error(
            "inria::Chrono::mean: Unknown name: " 
            + name);
    }
...
double Chrono::last(const std::string& name) const
{
    if (
        ...
        throw std::logic_error(
            "inria::Chrono::last: Unknown name: " 
            + name);
    }
