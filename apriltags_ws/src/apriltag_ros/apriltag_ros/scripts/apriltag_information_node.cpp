...
int main(int argc, char **argv)
{
    ...
    ros::Publisher tag20_publisher = n.advertise<apriltag_ros::AprilTagDetection>( "tag20/information", 1 );
    ...
    ros::Publisher tag20_filter_publisher = n.advertise<apriltag_ros::AprilTagDetection>( "tag20/filter", 1 );
    ...
    while(ros::ok())
    {
        ...
        robot_hand_msg.detected.data = false;
        ...
        for (int i = 0; i < tags.detections.size(); i++)
        {
            ...
            else if (tag.id[0] == 20 )
            {
                ...
                std::cout << "Tag20 detected" << std::endl;
                //last center position and orientation
        ...
        tag10_filter.detected.data = count_tag10 >= 18 ? true : false;
        ...
        if (robot_hand_estimation_tags_size != 0)
        {
            ...
            if(tag_detected == 5 and tag5_filter.detected.data)
            {
                ...
                Eigen::Matrix<double,3,1> p_next{0, 0, -0.046482};
            ...
            else if(tag_detected == 6 and tag6_filter.detected.data)
            {
                ...
                Eigen::Matrix<double,3,1> p_next{0, 0.046482, 0};
                Eigen::Matrix<double,3,1> p_new = r*p_next + p_now;
        ...
        if(tag5_filter.detected.data || tag6_filter.detected.data || tag7_filter.detected.data || tag8_filter.detected.data ){
            ...
            std::cout << "Robot hand detected" << std::endl;
            robot_hand_msg.pose = robot_hand;
            robot_hand_msg.detected.data = true;
        }

        pose_publisher.publish(robot_hand_msg);
        rh_pose_publisher.publish(robot_hand);
