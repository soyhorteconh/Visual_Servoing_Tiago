...
int main (int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        // ---------------------------------------------------------------------------------------------------- //
        // detecting values from camera once
        ...
        if(flag == true)
        {
            ...
            Eigen::Matrix<double,3,1> p_next{0, 0.0, 0.30};
            Eigen::Matrix<double,3,1> new_tag_position = tag_orientation_cf*p_next + tag_position_cf;

            // Eigen::Matrix<double, 3, 1> rest_5(0.0,0.0,0.15);
            ...
            // ---------------------------------------------------------------------------------------------------- //
            // transformation matrix for simulation
            // updating position
            ...
            Eigen::Matrix<double, 3, 1> new_position = - robot_hand_orientation_cf.transpose().inverse() * target_position_rhf + new_tag_position;
            ...
            target.pose.position.x = tag_position_cf_sim.x();
            target.pose.position.y = tag_position_cf_sim.y();
            target.pose.position.z = tag_position_cf_sim.z();
            ...
            //  target
            publish_marker("target", 1, "blue", tag_position_cf_sim, tag_quaternion_cf); 
            // initial
            //publish_marker("initial_position", 2, "green", robot_hand_position_cf_sim, robot_hand_quaternion_cf);
            // controller
            //publish_marker("robot_hand_position", 3, "red", robot_hand_position_cf, robot_hand_quaternion_cf);
        }
        std::cout << "--------------------------------" << std::endl;
        ros::spinOnce();
        loop_rate.sleep();
