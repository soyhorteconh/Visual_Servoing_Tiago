...
#include <random>
#include <Eigen/Dense>
#include <inria_maths/SplineCubic.hpp>
#include <inria_maths/FilterExponential.hpp>
#include <inria_maths/FilterIIR.hpp>
#include <inria_maths/FilterDifferentiator.hpp>
#include <inria_maths/FilterDifferentiator2.hpp>
#include <inria_maths/FilterBacklash.hpp>
...
static void testFilterExponential()
{
    ...
    inria::FilterExponential<double> filter1;
    inria::FilterExponential<double> filter2;
    filter1.reset(0.0);
    ...
    //sampling frequency
    inria::FilterExponential<double> filter3;
    inria::FilterExponential<double> filter4;
...
{
    inria::FilterIIR<double> filter1;
    inria::FilterIIR<double> filter2;
    inria::FilterIIR<double> filter3;
    inria::FilterIIR<double> filter4;
    filter1.init(
...
void testCompareExponantialAndIIR()
{
    ...
    inria::FilterExponential<double> filterEXP;
    filterEXP.cutoffFrequency() = 10.0;
    inria::FilterIIR<double> filterIIR1;
    filterIIR1.init(
        mkfilter::FilterType_t::Bessel,
        mkfilter::PassType_t::Lowpass, 0.0,
        1, 1.0/0.005, 10.0, 0.0);
    inria::FilterIIR<double> filterIIR3;
    filterIIR3.init(
        mkfilter::FilterType_t::Bessel,
        mkfilter::PassType_t::Lowpass, 0.0,
...
void testFilterDifferentiator()
{
    ...
    inria::FilterDifferentiator filterDiff1(2, 1.0/0.005);
    inria::FilterDifferentiator filterDiff2(6, 1.0/0.005);
    inria::FilterIIR<double> filterIIR1;
    inria::FilterIIR<double> filterIIR2;
    filterIIR1.init(
        mkfilter::FilterType_t::Bessel,
        mkfilter::PassType_t::Lowpass, 0.0,
    ...
    inria::FilterIIR<double> filterIIRPre;
    filterIIRPre.init(
        mkfilter::FilterType_t::Bessel,
        mkfilter::PassType_t::Lowpass, 0.0,
...
{
    //Test noisy signal differentiation
    inria::FilterDifferentiator2<double> filterDiff;
...
{
    inria::FilterBacklash filter(0.05);
    inria::Plot plot;
    for (double t=0.0;t<2.0;t+=0.005) {
        double target = 
...
void testFilterBangBangAcc()
{
    ...
    inria::FilterBangBangAcc<double> filterBangBangAcc1;
    inria::FilterBangBangAcc<Eigen::Vector3d> filterBangBangAcc2;
...
void testFilterInterpolation()
{
    inria::FilterInterpolation<double> filter;
    filter.reset(0.0, 1.0);
    inria::Plot plot;
    double offset = 0.0;
    for (double t=0.0;t<10.0;t+=0.01) {
        double raw = std::cos(2.0*M_PI*t) + offset;
        if (t > 4.8 && offset < 1e-3) {
            filter.reset(filter.value(), 1.0);
            offset = 2.0;
        }
        filter.update(raw, 0.01);
        plot.add(
            "time", t,
            "raw", raw,
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher publisher = n.advertise<std_msgs::Float64>( "filter", 1 );
    ...
    while(ros::ok())
    {
        ...
        for (double t=0.0;t<3.0;t+=0.001) {
            ...
            pubMsg.data = filter4.value();
            publisher.publish(pubMsg);
        }
        //plot.plot("time", "all").show();

        //double filter_value = filter4.value();
